...
namespace SC.Repositories.Data
{
    public class KeluhanRepository : IKeluhanRepository
    {
        ...
        {
            var data = myContext.Keluhans.Include(x => x.User.ProfilMahasiswa).ToList();
            return data;
        }

        public Keluhan Get(int id)
        {
            var data = myContext.Keluhans.Include(x => x.User.ProfilMahasiswa).Where(x => x.Id.Equals(id)).FirstOrDefault();
            return data;
        }
